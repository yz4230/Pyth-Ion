...
def detect_cusum(
    ...
    if maxstates == 0:
        ...
        def central_moving_average(data0, oneside_window):
            ...
            data = (data0 / data0_absmax * int32_max).astype(
                np.int64
            )  # convert to integers to avoid floating point error
            cumsum = np.cumsum(data)
            res = np.zeros_like(data)
            res[: oneside_window + 1] = cumsum[oneside_window : 2 * oneside_window + 1]
            res[oneside_window + 1 : -oneside_window] = (
                cumsum[2 * oneside_window + 1 :] - cumsum[: -2 * oneside_window - 1]
            )
            res[-oneside_window:] = (
                cumsum[-1] - cumsum[-2 * oneside_window - 1 : -oneside_window - 1]
            )
            res = res.astype(np.float64) / int32_max * data0_absmax
            res[: oneside_window + 1] /= np.arange(
                oneside_window + 1, 2 * oneside_window + 2
            )
        ...
        edges = np.array(
            [0], dtype="int64"
        )  # initialize an array with the position of the first subevent - the start of the event
        ...
        while k < length - 1:
            ...
            logp = (
                ...
                * basesd
                / variance
                * (data[k] - mean - stepsize * basesd / 2.0)
