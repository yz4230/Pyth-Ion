...
class ExtAppMainWindow(BaseAppMainWindow):
    def __init__(self, width, height, master=None):
        ...
        self.ui.actionDelete_Last_Cut_Region_Alt_D.triggered.connect(
            lambda: Selections.deleteOneCutLR(self)
        )
        self.ui.actionClear_Selections.triggered.connect(
            lambda: Selections.clearLRs(self)
        )
        self.ui.actionAuto_Select_IV_Region.triggered.connect(
            lambda: Selections.autoIVSelection(self)
        )
        self.ui.actionSelection_Export_Data.triggered.connect(
            lambda: IO.exportSelection(self)
        )
        self.ui.actionExport_measurement.triggered.connect(
            lambda: Selections.measureSelections(self)
        )
        self.ui.actionInspect_Selection.triggered.connect(
            lambda: Painting.inspectSelection(self)
        )

        self.ui.actionAnalyze.triggered.connect(self.doAnalysis)
        self.ui.actionSubevent_State_Settings.triggered.connect(
            self.setSubEventStateSettingsDialog
        )
        self.ui.actionRepaint_Analysis.triggered.connect(
        ...
        for p in self.p2s:
            for entry in self.scatter_entries:
                p[entry].sigClicked.connect(
                    lambda a, b: Painting.scatterClicked(self, a, b)
                )
