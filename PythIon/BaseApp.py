...
class BaseAppMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, width, height, master=None):
        ...
        for entry in self.scatter_entries:
            ...
            self.w1.addItem(p)
            self.p2[entry] = p
        self.w1.setLabel("bottom", text="Log Dwell Time", units="Log10(μs)")
        ...
        for entry in self.scatter_entries:
            ...
            self.p2std[entry] = p
        self.w1std.setLabel("bottom", text="Log Dwell Time", units="Log10(μs)")
        ...
        for entry in self.scatter_entries:
            ...
            self.p2skew[entry] = p
        self.w1skew.setLabel("bottom", text="Log Dwell Time", units="Log10(μs)")
        ...
        for entry in self.scatter_entries:
            ...
            self.p2kurt[entry] = p
        self.w1kurt.setLabel("bottom", text="Log Dwell Time", units="Log10(μs)")
    ...
    def ui_baseline(self, val):
        ...
        self.ui.eventcounterlabel.setText(
            f"Baseline={self.perfiledata.baseline * 1e9:.4f} nA"
        )
    ...
    def ui_baseline_std(self, val):
        ...
        self.printlog(
            f"Updated baseline std to {self.perfiledata.baseline_std * 1e9:.6f} nA"
        )
    ...
    def updateThresholdLine(self):
        ...
        self.perfiledata.threshHandle = self.p1.addLine(
            y=self.analysis_config.threshold_A * dscl, pen="r"
        )
...
class AutoSelectIVDialog(QtWidgets.QDialog):
    ...
    def dialogAccept(self):
        ...
        params = {"offset_ms": offset_ms, "start_ms": start_ms, "end_ms": end_ms}
        self.close()
