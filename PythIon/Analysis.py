...
class AnalysisResults:
    def __init__(self, analysis_config: Config):
        ...
        self.result_nullvalue = np.array(
            tuple(np.nan if spec[1] is float else -1 for spec in self.result_spec),
...
class SubEventStateSettingsDialog(QtWidgets.QDialog):
    def __init__(self, parent: BaseAppMainWindow):
        ...
        self.ui.lineEdit_MergeBlockadeThresh.setText(
            str(analysis_config.merge_delta_blockade)
        )
        self.ui.lineEdit_WinOfSmoothing.setText(str(analysis_config.prefilt_window_us))
        self.ui.lineEdit_MinStateDuration.setText(
            str(analysis_config.state_min_duration_us)
        )
    ...
    def dialogAccept(self):
        ...
        analysis_config.merge_delta_blockade = float(
            self.ui.lineEdit_MergeBlockadeThresh.text()
        )
        analysis_config.prefilt_window_us = float(
            self.ui.lineEdit_WinOfSmoothing.text()
        )
        analysis_config.state_min_duration_us = float(
            self.ui.lineEdit_MinStateDuration.text()
        )
        self.close()
...
class AnalyzeDialog(QtWidgets.QDialog):
    ...
    def unitChanged(self):
        ...
        if self.ui.radioButton_nA.isChecked():
            ...
            self.ui.lineEdit_threshold.setText(
                str(
                    analysis_config.threshold_A / self.parent.perfiledata.baseline * 1e2
                )
            )
    ...
    def valueChanged(self):
        analysis_config: Config = self.parent.analysis_config
        if self.ui.radioButton_nA.isChecked():
            analysis_config.threshold_A = (
                float(self.ui.lineEdit_threshold.text()) * 1e-9
            )
        elif self.ui.radioButton_pctBaseline.isChecked():
            analysis_config.threshold_A = (
                float(self.ui.lineEdit_threshold.text())
                / 1e2
                * self.parent.perfiledata.baseline
            )
    ...
    def dialogAccept(self):
        ...
        analysis_config.enable_subevent_state_detection = (
            self.ui.checkBox_enableSubeventStateDetection.isChecked()
        )
...
def cusum_worker_(
    ...
    cusum_res = detect_cusum(
        ...
        maxstates=cusum_maxNsta,
        stepsize=cusum_stepsize,
        threshhold=cusum_threshhold,
...
def computeAnalysis(app: BaseAppMainWindow):
    ...
    prefilt_oneside_window = int(
        prefilt_window_us / 2 * app.perfiledata.ADC_samplerate_Hz * 1e-6
    )
    ...
    for k in tqdm(range(app.perfiledata.data.Nseg), desc="Segment->Event"):
            ...
            while seg_filt[ep] < highthresh:
                ...
                if ep == len(seg_filt) - 1:  # sure that the current returns to baseline
                    ...
                    try:
                        if (
                            ep > startpoints[j + 1]
                        ):  # if we hit the next startpoint before we
                            startpoints[j + 1] = (
                                0  # return to baseline, mark for deletion
                            )
        ...
        for i in range(seg_numberofevents):
            mins = np.array(
                signal.argrelmin(seg_filt[startpoints[i] : endpoints[i]])[0]
                + startpoints[i]
            )
            ...
            mins = mins[
                seg_filt[mins]
                < (
                    analysis_config.baseline_A
                    + np.mean(seg_filt[startpoints[i] : endpoints[i]])
                )
                / 2
            ]
            if len(mins) == 1:
                pass
                seg_deli_list[i] = analysis_config.baseline_A - min(
                    seg_filt[startpoints[i] : endpoints[i]]
                )
                seg_dwell_list[i] = (
                ...
                seg_deli_list[i] = analysis_config.baseline_A - np.mean(
                    seg_filt[mins[0] : mins[-1]]
                )
                endpoints[i] = mins[-1]
                seg_dwell_list[i] = (
                    (endpoints[i] - startpoints[i])
        ...
        seg_dt_list = np.append(
            seg_dt_list, np.diff(startpoints) / app.perfiledata.ADC_samplerate_Hz
        )
        seg_numberofevents = len(startpoints)
        seg_noise = np.array(
            [np.std(seg_filt[x : endpoints[i]]) for i, x in enumerate(startpoints)]
        )
        seg_skew = np.array(
            [spstat.skew(seg_filt[x : endpoints[i]]) for i, x in enumerate(startpoints)]
        )
        seg_kurt = np.array(
            [
                spstat.kurtosis(seg_filt[x : endpoints[i]])
        ...
        if seg_numberofevents > 0:
            seg_result_table = np.full(
                seg_numberofevents, result_nullvalue, dtype=result_dtype
            )
            ...
            seg_result_table["mean"] = np.array(
                [np.mean(seg_filt[x : endpoints[i]]) for i, x in enumerate(startpoints)]
            )
        ...
        if analysis_config.enable_subevent_state_detection:
            ...
            shared_seg_filt = np.ndarray(
                seg_filt.shape, dtype=seg_filt.dtype, buffer=shm.buf
            )
            ...
            mo_cusum_res_list = list(
                tqdm(
                    mo_cusum_res_iter,
                    total=len(startendpoints_list),
                    desc="Event->State",
                    leave=False,
                )
            )
            mo_cusum_res_list = [
                (event_kx_list[k], mo_cusum_res)
                for k, mo_cusum_res in enumerate(mo_cusum_res_list)
            ]
            shm.close()
            shm.unlink()
            ...
            for res_item in mo_cusum_res_list:
                ...
                if cusum_Nsta > 1:
                    event_state_result_table = np.full(
                        cusum_Nsta, result_nullvalue, dtype=result_dtype
                    )
                    for ksta in range(cusum_Nsta):
                        ...
                        state_result["seg"] = k
                        state_result["local_startpt"] = (
                            state_start + first_min_offset + startpoint
                        )
                        state_result["local_endpt"] = (
                            state_end + first_min_offset + startpoint
                        )
                        state_result["global_startpt"] = (
                            state_result["local_startpt"] + seg_range[0]
                        )
                        state_result["global_endpt"] = (
                            state_result["local_endpt"] + seg_range[0]
                        )
                        state_result["mean"] = np.mean(state_data)
                        state_result["deli"] = (
                            analysis_config.baseline_A - state_result["mean"]
                        )
                        state_result["frac"] = (
                            state_result["deli"] / analysis_config.baseline_A
                        )
                        state_result["dwell"] = (
                            len(state_data) / app.perfiledata.ADC_samplerate_Hz * 1e6
                        )
                    ...
                    state_result_table = np.append(
                        state_result_table, event_state_result_table
                    )
