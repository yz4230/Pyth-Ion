...
def autoFindCutLRs(app: BaseAppMainWindow):
    with app.awaitresponse:
        for k in range(app.perfiledata.data.Nseg):
            ...
            ndx_spike = np.where(
                np.abs(seg_filt)
                > app.ui_baseline + std_thersh_for_spike * app.ui_baseline_std
            )[0]
            ndx_around_baseline = np.nonzero(
                np.abs(seg_filt - app.ui_baseline) < app.ui_baseline_std
            )[0]
            extra_relaxation_time_ms = 10
            Nrlx = int(
                extra_relaxation_time_ms * app.perfiledata.ADC_samplerate_Hz / 1e3
            )
            ...
            for startpoint in ndx_spike:
                if startpoint < endpoint:
                    continue
                k_nab += np.searchsorted(
                    ndx_around_baseline[k_nab:], startpoint, side="right"
                )
...
def addOneManualCutLR(app: BaseAppMainWindow):
    with app.awaitresponse:
        ...
        newLR.hide()
        newLR.setRegion(
            (lambda x: (0.7 * x[0] + 0.3 * x[1], 0.3 * x[0] + 0.7 * x[1]))(
                app.p1.viewRange()[0]
            )
        )
        app.p1.addItem(newLR)
...
def measureSelections(app: BaseAppMainWindow):
    with app.awaitresponse:
        if len(app.perfiledata.LRs) > 0:
            regions = [
                np.round(LR.getRegion()).astype(int) for LR in app.perfiledata.LRs
            ]
            ...
            savepath = (
                app.perfiledata.matfilename + "_" + timestamp + "_measurement.txt"
            )
            np.savetxt(
                savepath,
                measure_result,
                ...
                for LR in app.perfiledata.LRs:
                    ...
                    for t_V_ndx in range(*t_V_ndcs):
                        ...
                        if not (t_V_ndx > 0 and V == t_V["mV"][t_V_ndx - 1]):
                            # if True:
                            new_region_ms.append(
                                np.array([t + params["start_ms"], t + params["end_ms"]])
                            )
                app.clearLRs()
                # new_region_ndx = np.round(np.array(new_region_ms) * app.outputsamplerate).astype(int)
                new_region_ndx = (
                    1e-3 * np.array(new_region_ms) * app.perfiledata.ADC_samplerate_Hz
                )
